...
export function getReadinessProfile(pillarScores: PillarScore[], result: AssessmentResult, lang: Lang = DEFAULT_LANG): ReadinessProfile {
  ...
  if (result.score >= 75) {
    return lang === "en" ? {
      ...
      implication: `Bringing ${weakestTitle} up to the same standard as the rest extends the return already being generated by the other capabilities.`,
    } : {
      id: "strong-with-focus",
      ...
      implication: `Elevar ${weakestTitle} ao mesmo padrão dos demais amplia ainda mais o retorno que as outras capacidades já vêm gerando.`,
    };
  }
